...
class Order
{
    ...
    public function findById($id)
    {
        ...
        if ($order) {
            $order['billing_address'] = !empty($order['billing_address']) ? json_decode($order['billing_address'], true) : [];
            $order['shipping_address'] = !empty($order['shipping_address']) ? json_decode($order['shipping_address'], true) : [];
